...
fn init(@builtin(global_invocation_id) global_id: vec3<u32>, @builtin(num_workgroups) num_workgroups: vec3<u32>) {
    ...
    if (d <= b) {
        ...
        let index = get_index_by_invoc_id(global_id);
        ...
        for (var i: u32 = 0; i < 2; i++) {
            for (var j: u32 = 0; j < 2; j++) {
                for(var k: u32 = 0; k < 2; k++) {
                    ...
                    let pos = get_sub_pos_by_v_id(point, vid);
                    let d2 = map(pos);
                    if (d <= 0.) {
                        let vidx = get_index_by_invoc_id(vid);

                        voxels[index + vidx].col = 1u;
                        voxels[index + vidx].mat = 1u;

                        insertBits(mask, 1, vidx, 1);
                    }
                }
            }
        }
...
fn finalize(@builtin(global_invocation_id) global_id: vec3<u32>, @builtin(num_workgroups) num_workgroups: vec3<u32>) {
    ...
    let upper_idx = calculate_full_depth(global_id.x + 1) - 1;

    let index = upper_idx + get_index_by_invoc_id(global_id);
...
fn calculate_full_depth(depth: u32) -> u32 {
    return u32((pow(8., (f32(depth) + 1.)) - 1.) / 7.);
}
...
fn calc_pos_from_invoc_id(g: vec3<u32>, d: vec3<u32>) -> vec3<f32> {
    ...
    return vec3<f32>(
        (f32(g.x) / (f32(d.x) - 1.) - 0.5) * grid_size,
        (f32(g.y) / (f32(d.y) - 1.) - 0.5) * grid_size,
        (f32(g.z) / (f32(d.z) - 1.) - 0.5) * grid_size
    );
}

// get index by invoc_id
fn get_sub_pos_by_v_id(p: vec3<f32>, li: vec3<u32>) -> vec3<f32> {

    let grid_size = 0.5;

    // same as calc_pos_from_invoc_id just with hard coded dims
    return p + vec3<f32>(
        (f32(li.x) / 0.5) * (grid_size / 2.),
        (f32(li.y) / 0.5) * (grid_size / 2.),
        (f32(li.z) / 0.5) * (grid_size / 2.)
    );
}

fn get_index_by_invoc_id(g: vec3<u32>) -> u32 {
