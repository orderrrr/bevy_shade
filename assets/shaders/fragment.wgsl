...
fn distance_to_octree(p: vec3<f32>, i: u32) -> f32 {
    ...
    let p1 = calc_pos_from_invoc_id(grid_pos, d);

    var h: f32  = cube(p - p1, vec3((settings.scale / f32(d)) / 2.0));

    // if h < (settings.scale / f32(d) + 0.4) {
    //     h = cube_frame(p - p1, vec3(settings.scale / f32(d)), 0.01);
    // }
        ...
        for (var l: u32 = 0; l < depth; l++) {
            for (var m: u32 = 0; m < depth; m++) {
                for (var n: u32 = 0; n < depth; n++) {
                    ...
                    h = min(cube_frame(p - ip, vec3((settings.scale / f32(depth)) / 2.0), 0.01), h);
                }
            }
        }
...
fn calc_pos_from_invoc_id(block_indices: vec3<u32>, depth: u32) -> vec3<f32> {
    return (vec3<f32>(block_indices)) * (settings.scale / pow(2., f32(depth)) * 0.5);
}
